defmodule OwnershipAshChat.AiAgentActorPersister do
  use AshOban.ActorPersister

  def store(%OwnershipAshChat.Accounts.User{id: id}), do: %{"type" => "user", "id" => id}

  def lookup(%{"type" => "user", "id" => id}) do
    with {:ok, user} <- Ash.get(OwnershipAshChat.Accounts.User, id, authorize?: false) do
