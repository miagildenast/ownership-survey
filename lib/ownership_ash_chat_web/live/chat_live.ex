defmodule OwnershipAshChatWeb.ChatLive do
  use Elixir.OwnershipAshChatWeb, :live_view
  @actor_required? true
  @chat_ui_tools AshAi.ChatUI.Tools
  on_mount {OwnershipAshChatWeb.LiveUserAuth, :live_user_required}
  ...
  def mount(_params, _session, socket) do
    ...
    if socket.assigns.current_user do
      OwnershipAshChatWeb.Endpoint.subscribe(
    ...
    conversations =
      if @actor_required? && is_nil(socket.assigns.current_user) do
        ...
        OwnershipAshChat.Chat.my_conversations!(actor: socket.assigns.current_user)
      end
  ...
  def handle_params(%{"conversation_id" => conversation_id}, _, socket) do
    if @actor_required? && is_nil(socket.assigns.current_user) do
      ...
      conversation =
        OwnershipAshChat.Chat.get_conversation!(conversation_id,
          actor: socket.assigns.current_user
        )

      messages = OwnershipAshChat.Chat.message_history!(conversation.id, stream?: true)
      ...
      cond do
        ...
        socket.assigns[:conversation] ->
          OwnershipAshChatWeb.Endpoint.unsubscribe(
            "chat:messages:#{socket.assigns.conversation.id}"
          )

          OwnershipAshChatWeb.Endpoint.subscribe("chat:messages:#{conversation.id}")
  ...
  def handle_params(_, _, socket) do
    if socket.assigns[:conversation] do
      OwnershipAshChatWeb.Endpoint.unsubscribe("chat:messages:#{socket.assigns.conversation.id}")
    end
  ...
  defp assign_message_form(socket) do
    form =
      if socket.assigns.conversation do
        OwnershipAshChat.Chat.form_to_create_message(
          actor: socket.assigns.current_user,
          private_arguments: %{conversation_id: socket.assigns.conversation.id}
        )
        |> to_form()
      else
        OwnershipAshChat.Chat.form_to_create_message(actor: socket.assigns.current_user)
        |> to_form()
      end
